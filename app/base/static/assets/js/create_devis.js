...
var KTAppInvoicesCreate = function () {
    ...
    var handleEmptyState = function () {
        if (form.querySelectorAll('[data-kt-element="items"] [data-kt-element="item"]').length === 0 && form.querySelectorAll('[data-kt-element="items"] [data-kt-element="chapter"]').length === 0) {
    ...
    var handeForm = function (element) {
        ...
        form.querySelector('[data-kt-element="items"] [data-kt-element="add-item"]').addEventListener('click', function (e) {
            ...
            var add_subitem_button = form.querySelector('[data-kt-element="add-subitem-template"] tr').cloneNode(true);

            form.querySelector('[data-kt-element="items"] tbody').appendChild(item);
            form.querySelector('[data-kt-element="items"] tbody').appendChild(add_subitem_button);
        ...
        // Add chapter
        KTUtil.on(form, '[data-kt-element="items"] [data-kt-element="add-chapter"]', 'click', function (e) {
            e.preventDefault();

            var item = form.querySelector('[data-kt-element="chapter-template"] tr').cloneNode(true);
        ...
        // Add subitem
        KTUtil.on(form, '[data-kt-element="items"] [data-kt-element="add-subitem"]', 'click', function (e) {
            e.preventDefault();

            var item = form.querySelector('[data-kt-element="item-template"] tr').cloneNode(true);

            let $item = $(item);
            $item.children().first().toggleClass('pl-7 pl-12');
            form.querySelector('[data-kt-element="items"] tbody').appendChild(item);
